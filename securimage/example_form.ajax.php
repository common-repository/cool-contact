...
<head>
    ...
    <style type="text/css">
    <!--
        #success_content { border: 1px solid #000; width: 550px; text-align: left; padding: 10px 7px; background: #33ff33; color: #000; font-weight; bold; font-size: 1.2em; border-radius: 4px; -moz-border-radius: 4px; -webkit-border-radius: 4px; }
        fieldset { width: 90%; }
...
<p class="note">
  ...
  This example form also demonstrates how to submit a form to itself to display error contents.
</p>

<div id="success_content" style="display: none">Your content has been sent!<br />We will contact you as soon as possible.</div>
...
<form method="post" action="" id="contact_form" onsubmit="return processForm()">
  ...
  <p>
    ...
    <textarea name="ct_content" rows="12" cols="60"></textarea>
  </p>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && @$_POST['do'] == 'contact') {
        ...
        foreach($_POST as $key => $value) {
            if (!is_array($key)) {
                ...
                if ($key != 'ct_content') $value = strip_tags($value);
        ...
        $content = @$_POST['ct_content']; // the content from the form
        ...
        if (isset($GLOBALS['DEBUG_MODE']) && $GLOBALS['DEBUG_MODE'] == false) {
            ...
            if (strlen($content) < 20) {
                ...
                $errors['content_error'] = 'Please enter a content';
            }
        }
        ...
        if (sizeof($errors) == 0) {
            ...
            $time       = date('r');
            $content = "A content was submitted from the contact form.  The following information was provided.<br /><br />"
                     . "Name: $name<br />"
                     . "Email: $email<br />"
                     . "URL: $URL<br />"
                     . "content:<br />"
            ...
            if (isset($GLOBALS['DEBUG_MODE']) && $GLOBALS['DEBUG_MODE'] == false) {
                // send the content with mail()
                mail($GLOBALS['ct_recipient'], $GLOBALS['ct_msg_subject'], $content, "From: {$GLOBALS['ct_recipient']}\r\nReply-To: {$email}\r\nContent-type: text/html; charset=ISO-8859-1\r\nMIME-Version: 1.0");
            }

            $return = array('error' => 0, 'content' => 'OK');
            die(json_encode($return));
        } else {
            $errmsg = '';
            foreach($errors as $key => $error) {
                // set up error contents to display with each field
                $errmsg .= " - {$error}\n";
            }

            $return = array('error' => 1, 'content' => $errmsg);
            die(json_encode($return));
        }
    } // POST
} // function process_si_contact_form()
